...
def main() -> None:
    ...
    async def browse_results(request: Request):
        """
        Rebuilt table view of search: indexed #, from temporary table in SQLite.
        Query params: run_id (load from SQLite) or url/q/data (legacy base64 links).
        Original search page is not shown; only this custom table. Waits for next command.
        """
        import base64
        import html as html_module
        from urllib.parse import unquote_plus

        from persistence.browse_results_repo import get_run as get_browse_run
...
{table_rows_html}
        ...
        # Prefer run_id: load temporary indexed table from SQLite and generate HTML.
        if run_id_param and conn_factory:
            run_data = get_browse_run(conn_factory, run_id_param)
        ...
        # Legacy: data= base64 JSON of links.
        if not data_param:
            return HTMLResponse(
